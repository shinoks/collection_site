...
class Menu
{
     ...
     * @return Men|null
     */
    public function getActiveMainColorMenu(): ?Men
    {
        $url = urldecode($this->requestStack->getCurrentRequest()->getPathInfo());
        if($url =='/'){$url = '/index';};
        $active = $this->em
            ->getRepository(Men::class)
            ->findOneByActiveUrlAndInMainColor($url);
        if($active){
            $activeMenu = $active;
        }else {
            $activeMenu = NULL;
     ...
     * @return array|null
     */
    public function getMainColorMenu(): ?array
    {
        $menu = $this->em->getRepository(Men::class)->findBy(['isActive' => 1, 'inMainColor' => 1, 'parent' => null],['position' => 'ASC', 'id' => 'DESC']);
        ...
        if($menu){
            foreach($menu->getModules() as $module){
                    ...
                    if($module->getPosition()->getPosition() == 'top'){
                        $modules['top'][$i] = ['position' =>$module->getPosition()->getPosition(), 'id'=>$module->getId(),'content' => $module->getGenus()->getContent(), 'template' => $module->getGenus()->getTemplate(), 'sequence' => $module->getSequence(), 'variable' => $module->getVariable(), 'name' => $module->getName()];
                    }elseif($module->getPosition()->getPosition() == 'bottom'){
                        $modules['bottom'][$i] = ['position' =>$module->getPosition()->getPosition(), 'id'=>$module->getId(),'content' => $module->getGenus()->getContent(), 'template' => $module->getGenus()->getTemplate(), 'sequence' => $module->getSequence(), 'variable' => $module->getVariable(), 'name' => $module->getName()];
                    }
                    $i = $i+1;
                }
            }
