            ...
            if($recruitmentStatus){
                ...
                if($recruitmentStatus->getIsMailedToAdmin() == 1){
                    ...
                    foreach($admins as $admin){
                        ...
                        $mailManager->sendEmail($mailBodyPersonalized,['subject' => $config->getTitle().' - Status inwestycji uległ zmianie'],$admin->getEmail(),$mailer,NULL);
                    }
                }
                if($recruitmentStatus->getIsMailedToUsers() == 1){
                    ...
                    foreach($users as $user){
                        ...
                        $mailBodyPersonalized = str_replace('user',$name, $mailBody);
                        $mailManager->sendEmail($mailBodyPersonalized,['subject' => $config->getTitle().' - Status inwestycji uległ zmianie'],$user->getEmail(),$mailer,NULL);
                    }
                }
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if(!$usersActive){
                ...
                foreach($usersActive as $user){
                    ...
                    $mailManager->sendEmail($mailBodyPersonalized,['subject' => $config->getTitle().'- Nowa inwestycja'],$user->getEmail(),$mailer);
                }
            }
