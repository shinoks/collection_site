...
class RecruitmentUserController extends Controller
{
        ...
        if($recruitmentUser){
            ...
            if($recruitmentUserStatus){
                ...
                if($recruitmentUserStatus->getIsMailed() == 1){
                    ...
                    $mailManager->sendEmail($mailBodyPersonalized,['subject' => $config->getTitle().' - Status twojej oferty uległ zmianie'],$user->getEmail(),$mailer,$files);
                }
    ...
    public function getAgreement($id)
    {
        ...
        return $this->file($recruitmentUser->getAbsoluteAgreementPath(),$recruitmentUser->getNumber().'.pdf');
    }

    public function regenerateAgreement($id)
    {
        $recruitmentUser = $this->getDoctrine()
            ->getRepository(RecruitmentUsers::class)
            ->find($id);
        if($recruitmentUser){
            $this->generateAgreement($id);
